...
-export([start_link/3, start/3, get_master/2, call_master/3, cast_master/2]).
...
-spec call_master(atom(), term(), pos_integer()) ->
    term() | {error, no_master}.

call_master(Callback, Msg, Timeout) ->
...
%% @doc Cals to the current master
-spec cast_master(atom(), term()) ->
    ok | {error, no_master}.

cast_master(Callback, Msg) ->
