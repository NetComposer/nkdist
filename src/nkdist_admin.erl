...
get_info() ->
    [
        ...
        {vnodes, riak_core_vnode_manager:all_index_pid(nkdist_vnode)}
    ].
...
print_info() ->
    ...
    partitions(),
    % io:format("-------------------------- Idx2Num ------------------------------\n", []),
    % io:format("~p\n", [riak_core_mochiglobal:get(nkdist_idx2pos)]),
...
% ensemble_detail(N) ->
%     nkdist_ensemble_admin:ensemble_detail(N).
